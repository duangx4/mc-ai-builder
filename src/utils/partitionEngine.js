...
5. **屋顶规范（如果本区块含屋顶/顶部收口）**：
   - 禁止平顶：屋顶必须逐层内收的斜坡（每高 1 格水平内收 1 格，约 45°）
   - 至少两层：下层橡木梁/板支撑（檐口挑出墙外 1 格），上层瓦片/深灰混凝土
   - 顶部屋脊收成一条脊瓦，四角用橡木梁承接不悬空
   - 屋顶总高约为墙体高度的 30%~50%
${previousError ? `
6. 上一次构建失败，错误信息：${String(previousError).slice(0, 300)}
